...
//		MMatchInv matchInv = (MMatchInv)getPO();
		...
		/**
		 * ELTEK - INICIO
		 * Mario Grigioni - 04/12/2009, mgrigioni
		 */
		MInvoice inv = m_invoiceLine.getParent();
		...
		int LBR_DocType_Acct_ID = AdempiereLBR.getDocTypeAcct(io.getC_DocType_ID());
		if (LBR_DocType_Acct_ID != 0)
		{
			...
			// Se for uma venda
			if (isSOTrx()){
				int LBR_Acct_DR = DocAcct.getlbr_Acct_DR();
				if (LBR_Acct_DR != 0)
					account = new MAccount(getCtx(),LBR_Acct_DR,null);
			}
			// Senão é um recebimento
			else{
				int LBR_Acct_CR = DocAcct.getlbr_Acct_CR();
				if (LBR_Acct_CR != 0)
					account = new MAccount(getCtx(),LBR_Acct_CR,null);
			}
		}
		/** ELTEK - FIM **/
		...
		**/
		...
		//Account != null == LBR
		if (account != null){
			dr = fact.createLine(null, account, as.getC_Currency_ID(), Env.ONE,null);
		}
		else {
