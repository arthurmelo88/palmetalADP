 ...
 *
 *  OBS: CLASSE MIGRADA DO 360 PARA O PROJETO LBR POR PALMETAL
 *
 *
 */
public final class EMail implements Serializable
{
	...
	//Palmetal - begin
	/**
	 * metodo usado para envio de emails da Palmetal
	 */
	public EMail (Properties ctx, String smtpHost, String from, String to, 
			String subject, String message, String user, String pass)
		{
			setSmtpHost(smtpHost);
			setFrom(from);
			//setFrom("Palmetal - Alezzia");
			addTo(to);
			m_auth = createAuthenticator(user, pass);
			m_ctx = ctx;
			if (subject == null || subject.length() == 0)
				setSubject(".");	//	pass validation
			else
