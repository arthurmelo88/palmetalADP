...
public abstract class CreateFrom implements ICreateFrom
{
	...
	protected ArrayList<KeyNamePair> loadOrderData (int C_BPartner_ID, boolean forInvoice, boolean sameWarehouseOnly)
	{
		...
		//BEGIN 
		boolean isSOTrx = getGridTab().getValueAsBoolean("IsSOTrx");
		//END
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		//END	
			sql = new StringBuffer("SELECT "
					...
					+ " l.AD_Org_ID, o.Name " //	10..11
					+ "FROM C_OrderLine l"
					+ " INNER JOIN AD_Org o ON (l.AD_Org_ID = o.AD_Org_ID) "
				...
				//ELTEK: Adicionado o append abaixo
				if(forInvoice)
					sql.append(" AND l.QtyInvoiced < l.QtyEntered ");
				else
				...
				sql.append(" GROUP BY l.QtyOrdered,CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END, "
					...
					+ "l.M_Product_ID,COALESCE(p.Name,c.Name), l.Line,l.C_OrderLine_ID, l.AD_Org_ID, o.Name, po.VendorProductNo "
					+ "ORDER BY l.Line");
		//BEGIN
		}else{
			sql = new StringBuffer("SELECT "
					+ "l.QtyOrdered-COALESCE(l.QtyInvoiced,0),"					//	1
					+ "CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END,"	//	2
					+ " l.C_UOM_ID,COALESCE(uom.UOMSymbol,uom.Name),"			//	3..4
					+ " COALESCE(l.M_Product_ID,0),COALESCE(p.Name,c.Name),null,"	//	5..7
					+ " l.C_OrderLine_ID,l.Line, "							//	8..9
					+ " l.AD_Org_ID, o.Name " //	10..11
					+ "FROM C_Invoice_Candidate_v ic inner join C_OrderLine l ON (ic.c_order_id=l.c_order_id) "
