 ...
 */
public class CreateFromInvoice_OLD extends CreateFrom
{
	/**
	 *  Protected Constructor
	 *  @param mTab MTab
	 */
	public CreateFromInvoice_OLD(GridTab mTab)
	{
		super(mTab);
	...
	protected Vector<Vector<Object>> getShipmentData(int M_InOut_ID)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				//ELTEK: Inserida as duas linhas abaixo
...
//	    MRMA m_rma = new MRMA(Env.getCtx(), M_RMA_ID, null);
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(1, String.class, true);        //  1-AD_Org_ID
		miniTable.setColumnClass(2, BigDecimal.class, true);    //  2-Qty
		miniTable.setColumnClass(3, String.class, true);        //  3-UOM
		miniTable.setColumnClass(4, String.class, true);        //  4-Product
		miniTable.setColumnClass(5, String.class, true);        //  5-VendorProductNo
		miniTable.setColumnClass(6, String.class, true);        //  6-Order
		miniTable.setColumnClass(7, String.class, true);        //  7-Ship
		miniTable.setColumnClass(8, String.class, true);        //  8-Invoice
		//  Table UI
		miniTable.autoSize();
	}
		...
		*/
		...
		for (int i = 0; i < miniTable.getRowCount(); i++)
		{
			if (((Boolean)miniTable.getValueAt(i, 0)).booleanValue())
			{
				...
				//  variable values
				BigDecimal QtyEntered = (BigDecimal)miniTable.getValueAt(i, 2);              //  1-Qty

				pp = (KeyNamePair)miniTable.getValueAt(i, 3);   //  3-UOM
				...
				pp = (KeyNamePair)miniTable.getValueAt(i, 6);               //  6-OrderLine
				if (pp != null)
					C_OrderLine_ID = pp.getKey();
				int M_InOutLine_ID = 0;
				pp = (KeyNamePair)miniTable.getValueAt(i, 7);               //  7-Shipment
				if (pp != null)
					M_InOutLine_ID = pp.getKey();
				//
				int M_RMALine_ID = 0;
				pp = (KeyNamePair)miniTable.getValueAt(i, 8);               //  8-RMALine
				if (pp != null)
