...
/**
 * 		Utilizado para efetuar os c�lculos dos impostos brasileiros.
 * 
...
public class VLBRTax implements ModelValidator
{
	...
	{
		/**
		 * 	Calcula o imposto quando uma linha � salva
		 */
		if ((MOrderLine.Table_Name.equals(po.get_TableName()) || MInvoiceLine.Table_Name.equals(po.get_TableName())) 
				&& (type == TYPE_BEFORE_NEW || ( type == TYPE_BEFORE_CHANGE && isChangeAffectTaxes (po))))
		{
//			return calculateTaxesLine (po);
			String result = calculateTaxesLine (po);
			
			/**
			 * 	N�o fazer o update no n�vel do pedido para BEFORE NEW, porque as 
			 * 		linhas da OV ainda n�o foram salvas.
			 * 	Especificamente para novos registros o lan�amento �
		...
		/**
		 * 	Lan�amento dos impostos para registros novos, pois nestes casos
		 * 		as linhas ainda n�o foram salvas. Para registros alterados
		 * 		o lan�amento do imposto � feito no BEFORE SAVE.
		 * 	Tamb�m deve ser recalculado quando uma linha � deletada.
		 */
		else if ((type == TYPE_AFTER_NEW || type == TYPE_AFTER_DELETE))
			return updateTaxLine(po, true) ? null : "@LBR|ErrorSavingTaxes@";
		
		/**
		 * 	Calcula o imposto quando o cabe�alho � salvo
		 * 		e houve alguma altera��o que pode comprometer
	...
	private boolean isChangeAffectTaxes (PO po)
	{
		if (po instanceof MOrder || po instanceof MInvoice)
		{
			...
			if (po instanceof MOrder && (po.is_ValueChanged(I_W_C_Order.COLUMNNAME_Bill_BPartner_ID)
					|| po.is_ValueChanged(I_W_C_Order.COLUMNNAME_Bill_Location_ID) || po.is_ValueChanged(I_W_C_Order.COLUMNNAME_FreightCostRule) || po.is_ValueChanged(I_W_C_Order.COLUMNNAME_FreightAmt)))
				return true;
		}
	...
	{		
		...
		if (MOrder.Table_Name.equals(tableName))
		{
			 ...
			 * 		s�o adicionados no total, j� para a fatura
