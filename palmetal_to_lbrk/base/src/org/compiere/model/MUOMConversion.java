...
public class MUOMConversion extends X_C_UOM_Conversion
{
	...
	/**
	 *	Convert Qty expressed in entered UoM to equivalent price in product UoM and round. <br/>
	 *   For a Product
	 * @author fernandinho (fernando.moraes@faire.com.br)
	 * @param ctx
	 ...
	 * @return
	 */
	public static BigDecimal convertProductQty(Properties ctx,int p_M_Product_ID,int from_C_UOM_ID, int to_C_UOM_ID,BigDecimal value){
		if (value==null || value.compareTo(Env.ZERO)==0)
			return Env.ZERO;
		if (p_M_Product_ID==0 || from_C_UOM_ID==0 || to_C_UOM_ID==0)
			return Env.ZERO;
		if (from_C_UOM_ID==to_C_UOM_ID)
			return value;
		
		MProduct p = new MProduct(ctx,p_M_Product_ID,null);
		if (to_C_UOM_ID==p.getC_UOM_ID()){
			return convertProductFrom(ctx, p_M_Product_ID, from_C_UOM_ID, value);
		}else if (from_C_UOM_ID==p.getC_UOM_ID()){
			return convertProductTo(ctx, p_M_Product_ID, to_C_UOM_ID, value);
		}else{
			value = convertProductFrom(ctx, p_M_Product_ID, from_C_UOM_ID, value);// Convert From_C_UOM_ID to UOM Product
			value = convertProductTo(ctx, p_M_Product_ID, to_C_UOM_ID, value); // Convert UOM Product TO to_C_UOM_ID
	 ...
	 * @return
	 */
	public static BigDecimal convertProductPrice(Properties ctx,int p_M_Product_ID,int from_C_UOM_ID, int to_C_UOM_ID,BigDecimal value){
		if (value==null || value.compareTo(Env.ZERO)==0)
			return Env.ZERO;
		if (p_M_Product_ID==0 || from_C_UOM_ID==0 || to_C_UOM_ID==0)
			return Env.ZERO;
		...
		MProduct p = new MProduct(ctx,p_M_Product_ID,null);
		if (to_C_UOM_ID==p.getC_UOM_ID()){
			return convertProductTo(ctx, p_M_Product_ID, from_C_UOM_ID, value);
		}else if (from_C_UOM_ID==p.getC_UOM_ID()){
			return convertProductFrom(ctx, p_M_Product_ID, to_C_UOM_ID, value);

		}else{
			value = convertProductTo(ctx, p_M_Product_ID, from_C_UOM_ID, value);// Convert From_C_UOM_ID to UOM Product
			value = convertProductFrom(ctx, p_M_Product_ID, to_C_UOM_ID, value); // Convert UOM Product TO to_C_UOM_ID
