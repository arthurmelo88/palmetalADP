...
public class GridTab implements DataStatusListener, Evaluatee, Serializable
{
	...
	public String getTrxInfo()
	{
		...
		//	Nota Fiscal - Mario Grigioni (mgrigioni)
		else if (m_vo.TableName.startsWith("LBR_NotaFiscal"))
		{
			int Record_ID;
			//
			StringBuffer sql = new StringBuffer("SELECT COUNT(*) AS Lines, nf.TotalLines, nf.GrandTotal ");

			Record_ID = Env.getContextAsInt(m_vo.ctx, m_vo.WindowNo, "LBR_NotaFiscal_ID");
			...
			sql.append(" GROUP BY nf.GrandTotal, nf.TotalLines");
			...
			try
			{
				...
				if (rs.next())
				{
					...
					arguments[0] = lines;
					//	{1} - Line toral
					Double lineTotal = new Double(rs.getDouble(2));
					arguments[1] = lineTotal;
					//	{2} - Grand total (including tax, etc.)
					Double grandTotal = new Double(rs.getDouble(3));
					arguments[2] = grandTotal;
					//	{3} - Currency
					String currency = "BRL";
					arguments[3] = currency;
					//	(4) - Grand total converted to Euro
					Double grandEuro = new Double(rs.getDouble(3));
					arguments[4] = grandEuro;
					filled = true;
				}
			}
