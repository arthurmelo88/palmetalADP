...
public class MOrderLine extends X_C_OrderLine
{
	...
	public boolean isTaxIncluded()
	{
		...
		{
			//BEGIN faire @ faire
			//BUG - Qdo faz a chamada aqui primeiro antes do before save, o flag m_issoTrx = true, msm q seja um pedido de compra
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		MDocType dt = new MDocType (Env.getCtx(), getParent().getC_DocTypeTarget_ID(), null);

		//	Qty on instance ASI for SO
		if (m_IsSOTrx && (dt.getDocSubTypeSO() == null || !dt.getDocSubTypeSO().equals("RM"))
				...
				|| is_ValueChanged("M_Warehouse_ID")))
		{
			...
			if (product.isStocked())
			{
				...
				if (isInstance)
				{
					/* BF - https://sourceforge.net/tracker/index.php?func=detail&aid=3093321&group_id=278998&atid=1184347
					 * @edilsondneto
					 * 
...
//			setFreightAmt(Env.ZERO);
		...
		if (getQtyInvoiced().subtract(getQtyOrdered()).compareTo(Env.ZERO) > 0){

		log.saveError("QtyInsufficient", "=" + getQtyOrdered());
